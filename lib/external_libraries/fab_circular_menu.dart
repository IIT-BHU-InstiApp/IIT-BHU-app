...
class FabCircularMenu extends StatefulWidget {
  ...
  final List<Widget> innerRingChildren;
  final double innerRingDiameter;
...
// INNER RING
          Transform(
            ...
            alignment: FractionalOffset.center,
            child: OverflowBox(
              maxWidth: _innerRingDiameter,
              maxHeight: _innerRingDiameter,
              child: Container(
                width: _innerRingDiameter,
                height: _innerRingDiameter,
                child: CustomPaint(
                  painter:
                      _RingPainter(width: _innerRingWidth, color: _ringColor),
                  child: _scaleAnimation.value == 1.0
                      ? Transform.rotate(
                          angle: (2 * pi) *
                              _rotateAnimation.value *
                          ...
                          child: Container(
                            child: Stack(
                              alignment: Alignment.center,
                              children: widget.innerRingChildren
                                  .asMap()
                                  .map((index, child) => MapEntry(index,
                                      _applyTransformationsInner(child, index)))
                                  .values
                                  .toList(),
                            ),
                          ),
  ...
  Widget _applyTransformationsInner(Widget child, int index) {
    double angleFix = 0.0;
    ...
    return Transform(
        transform: Matrix4.translationValues(
            (-(_innerRingDiameter / 2) * cos(angle) +
                        (_ringWidth / 2 * cos(angle))) *
                    _directionX -
                8,
            (-(_innerRingDiameter / 2) * sin(angle) +
  ...
  void _calculateProps() {
    ...
    _ringDiameter =
        widget.ringDiameter ?? min(_screenWidth, _screenHeight) * 1.15;

    _innerRingDiameter =
        widget.innerRingDiameter ?? min(_screenWidth, _screenHeight) * 0.75;

    _ringWidth = widget.ringWidth ?? _ringDiameter * 0.3;
    _innerRingWidth = widget.innerRingWidth ?? _innerRingDiameter * 0.3;

    _marginH = (widget.fabMargin.right + widget.fabMargin.left) / 3;
    _marginV = (widget.fabMargin.top + widget.fabMargin.bottom) / 1.5;
